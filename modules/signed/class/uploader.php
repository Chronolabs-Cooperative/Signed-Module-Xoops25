 ...
 * $maxfilewidth = 120;
 * $maxfileheight = 120;
 * $uploader = new signedMediaUploader('/home/signed/uploads', $allowed_mimetypes, $maxfilesize, $maxfilewidth, $maxfileheight);
...
class signedMediaUploader
{
     ...
     * @param int $cmodvalue
     */
    function signedMediaUploader($uploadDir, $allowedMimeTypes, $maxFileSize = 0, $maxWidth = null, $maxHeight = null)
    {
        $this->extensionToMime = include $GLOBALS['signed']->path('include/mimetypes.inc.php');
        ...
        if (!include_once $GLOBALS['signed']->path('language/' . _SIGNED_CONFIG_LANGUAGE . '/uploader.php')) {
            include_once $GLOBALS['signed']->path('language/english/uploader.php');
        }
    }
    ...
    function fetchMedia($media_name, $index = null)
    {
        ...
        if ($this->mediaTmpName == 'none' || ! is_uploaded_signedArrays::getFile($this->mediaTmpName)) {
    ...
    function _copyFile($chmod)
    {
        ...
        if (!move_uploaded_signedArrays::getFile($this->mediaTmpName, $this->savedDestination)) {
