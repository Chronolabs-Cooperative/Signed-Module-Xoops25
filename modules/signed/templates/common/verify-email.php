		...
		if ($array['verification']['verified'] == false) {
			if ($array['verification']['emails'][$key]['verified'] == false) {
				if ($array['verification']['emails'][$key]['key'] == $hash) {
					...
					$array['verification']['emails'][$key]['verified-ip'] = json_decode(signedArrays::getFileContents("http://lookups.labs.coop/v1/country/".signedSecurity::getInstance()->getIP(true)."/json.api"), true);
